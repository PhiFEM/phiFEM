...
for i in range(20):
    ...
    try:
        from data import robin
        u_R.interpolate(robin)
    except ImportError:
        # In this case we assume the Robin condition coefficient is 1
        alpha = 1.
        ...
        u_R = ufl.inner(ufl.grad(exact_solution_h), grad_phi_h_normalized) + alpha * exact_solution_h
    ...
    penalization = penalization_coefficient * \
                 ...
                 + h_T**(-2) * ufl.inner(ufl.inner(y, ufl.grad(phi_h)) - ufl.inner(norm_grad_phi_h, alpha * u) + h_T**(-1) * ufl.inner(p, phi_h), ufl.inner(z, ufl.grad(phi_h)) - ufl.inner(norm_grad_phi_h, alpha * v) + h_T**(-1) * ufl.inner(q, phi_h)))
    ...
    rhs = ufl.inner(f_h, v)
    penalization_rhs = penalization_coefficient * (- h_T**(-2) * ufl.inner(u_R, ufl.sqrt(ufl.inner(ufl.grad(phi_h), ufl.grad(phi_h))) * (ufl.inner(z, ufl.grad(phi_h)) - ufl.inner(norm_grad_phi_h, alpha * v) + h_T**(-1) * ufl.inner(q, phi_h))) \
