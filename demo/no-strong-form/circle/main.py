...
nx = int(np.abs(bbox[1][0] - bbox[0][0]) / MESH_SIZE / np.sqrt(2.0))
ny = int(np.abs(bbox[1][1] - bbox[0][1]) / MESH_SIZE / np.sqrt(2.0))
bg_mesh = dfx.mesh.create_rectangle(MPI.COMM_WORLD, bbox, [nx, ny])
...
vector_element = element("Lagrange", cell_name, primal_degree, shape=(gdim,))
...
# Interpolation of the source terms
f_scalar_h = dfx.fem.Function(primal_space)
...
L = (
    ...
    + ufl.inner(f_vector_h, ufl.grad(v)) * dx((1, 2))
)
