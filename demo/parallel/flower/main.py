...
sys.path.append("/home/rbulle/Code/INRIA/phiFEM")
...
bg_mesh = dfx.mesh.create_rectangle(comm, bbox, [8, 8])
...
if mesh_type == "bg":
    cells_tags, facets_tags, _, d_bdy, _ = compute_tags_measures(
        bg_mesh, detection_levelset_h, 1, box_mode=True, single_layer_cut=True
    )
    mesh = bg_mesh
    ds = d_bdy(100)
elif mesh_type == "sub":
...
# plot_mesh(bg_levelset_space, cells_tags)
...
imap = dg0_space.dofmap.index_map
l2g_map = imap.local_to_global(np.arange(imap.size_local))
g2l_map = imap.global_to_local(np.arange(imap.size_global))

cells_tags_h = dfx.fem.Function(dg0_space)
ranks_h = dfx.fem.Function(dg0_space)
ranks_h.x.array[:] = rank
with XDMFFile(comm, "ranks.xdmf", "w") as of:
    of.write_mesh(mesh)
    of.write_function(ranks_h)

cells_tags_h.x.array[:] = cells_tags.values[:]

with XDMFFile(comm, "cells_tags.xdmf", "w") as of:
    of.write_mesh(mesh)
...
facets_tags_h = dfx.fem.Function(dg0_space_wf)
facets_tags_h.x.array[:] = facets_tags.values[:]
with XDMFFile(wireframe.comm, "facets_tags.xdmf", "w") as of:
